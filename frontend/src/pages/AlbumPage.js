...
const AlbumPage = () => {
    ...
    const mediaSrc = (upload) => upload.thumbnail_url || upload.file_url || upload.file;
        ...
        <div className="max-w-4xl mx-auto py-12 px-4">
            ...
            {uploads.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {uploads.map((upload) => (
                        <div key={upload.id} className="border rounded-lg p-4 flex flex-col">
                            <div className="mb-2">
                            ...
                            <div className="flex-1 mb-2">
                                {upload.file_type === 'image' || (upload.file_type && upload.file_type.startsWith('image')) ? (
                                    <img src={mediaSrc(upload)} alt={upload.original_filename} className="max-h-48 rounded" />
                                ) : upload.file_type === 'video' || (upload.file_type && upload.file_type.startsWith('video')) ? (
                                    <video src={upload.file_url || upload.file} controls className="max-h-48 rounded" />
                                ) : upload.file_type === 'audio' || (upload.file_type && upload.file_type.startsWith('audio')) ? (
                                    <audio src={upload.file_url || upload.file} controls />
                                ) : (
                                    <a href={upload.file_url || upload.file} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">
                                        Dosyayı görüntüle
                                    </a>
                                )}
                            </div>
                            {upload.caption && <div className="text-sm text-gray-600 mb-1">{upload.caption}</div>}
                            <div className="text-xs text-gray-400">Yükleyen: {upload.uploader_display_name || 'Anonim'}</div>
